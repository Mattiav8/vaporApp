 ...
 final class CategoryTests: XCTestCase {
    ...
    func testCategoriesCanBeRetrievedFromAPI() throws {
        ...
        let categories = try app.getResponde(to: categoriesURI, decodeTo: [App.Category].self)
    ...
    func testCategoryCanBeSavedWithAPI() throws {
        ...
        let receivedCategory = try app.getResponde(to: categoriesURI, method: .POST, headers: ["Content-Type": "application/json"], data: category, decodeTo: Category.self, loggedInRequest: true)
    ...
    func testGettingASingleCategoryFromTheAPI() throws {
        let category = try Category.create(name: categoryName, on: conn)
        let returnedCategory = try app.getResponde(to: "\(categoriesURI)\(category.id!)", decodeTo: Category.self)
    ...
    func testGettingACategoriesAcronymsFromTheAPI() throws {
        ...
        let acronyms = try app.getResponde(to: "\(categoriesURI)\(category.id!)/acronyms", decodeTo: [Acronym].self)
