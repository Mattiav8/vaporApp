...
final class AcronymTests: XCTestCase {
    ...
    func testAcronymCanBeSavedWithAPI() throws {
        ...
        let receivedAcronym = try app.getResponde(to: acronymsURI, method: .POST, headers: ["Content-Type": "application/json"], data: acronym, decodeTo: Acronym.self)
    ...
    func testGettingAnAcronymsUser() throws {
        ...
        let acronymsUser = try app.getResponde(to: "\(acronymsURI)\(acronym.id!)/user", decodeTo: User.self)
