...
final class AcronymTests: XCTestCase {
    ...
    func testAcronymsCanBeRetrievedFromAPI() throws {
        ...
        let acronyms = try app.getResponde(to: acronymsURI, decodeTo: [Acronym].self)
    ...
    func testAcronymCanBeSavedWithAPI() throws {
        ...
        let receivedAcronym = try app.getResponde(to: acronymsURI, method: .POST, headers: ["Content-Type": "application/json"], data: acronym, decodeTo: Acronym.self, loggedInRequest: true)
    ...
    func testGettingASingleAcronymFromTheAPI() throws {
        ...
        let returnedAcronym = try app.getResponde(to: "\(acronymsURI)\(acronym.id!)", decodeTo: Acronym.self)
    ...
    func testDeletingAnAcronym() throws {
        ...
        var acronyms = try app.getResponde(to: acronymsURI, decodeTo: [Acronym].self)
        
        XCTAssertEqual(acronyms.count, 1)
        
        _ = try app.sendRequest(to: "\(acronymsURI)\(acronym.id!)", method: .DELETE, loggedInRequest: true)
        acronyms = try app.getResponde(to: acronymsURI, decodeTo: [Acronym].self)
    ...
    func testSearchAcronymShort() throws {
        let acronym = try Acronym.create(short: acronymShort, long: acronymLong, on: conn)
        let acronyms = try app.getResponde(to: "\(acronymsURI)?term=OMG", decodeTo: [Acronym].self)
    ...
    func testSearchAcronymLong() throws {
        let acronym = try Acronym.create(short: acronymShort, long: acronymLong, on: conn)
        let acronyms = try app.getResponde(to: "\(acronymsURI)?term=Oh+My+God", decodeTo: [Acronym].self)
    ...
    func testGetFirstAcronym() throws {
        ...
        let firstAcronym = try app.getResponde(to: "\(acronymsURI)first", decodeTo: Acronym.self)
    ...
    func testSortingAcronyms() throws {
        ...
        let sortedAcronyms = try app.getResponde(to: "\(acronymsURI)sorted", decodeTo: [Acronym].self)
    ...
    func testGettingAnAcronymsUser() throws {
        ...
        let acronymsUser = try app.getResponde(to: "\(acronymsURI)\(acronym.id!)/user", decodeTo: User.Public.self)
    ...
    func testAcronymsCategories() throws {
        ...
        let categories = try app.getResponde(to: "\(acronymsURI)\(acronym.id!)/categories", decodeTo: [App.Category].self)
        ...
        let newCategories = try app.getResponde(to: "\(acronymsURI)\(acronym.id!)/categories", decodeTo: [App.Category].self)
